...
Hatchify enables you to make changes to your database schema and customize app behavior independently. When using code generation tools you have to write your schema and then generate your code, but once you start making customizations you cant re-run the generator without losing your customizations.

Hatchify is NOT code generation, its a system of modular and hierarchial libraries that can be consumed piecemeal to use as much or as little of Hatchifys abilities as you require.

# Quick Start Guide

Create a new Koa + Hatchify project. You can use `npm init` to create a new nodejs project.
...
Thats it! You have a basic CRUD application up and running. For more examples and detailed usage of what you can do with Hatchify, take a look at the more complete 'Getting Started Tutorial' below.
...
A project using Hatchify is generally made up of a few different parts, the backend service layer, the frontend display layer, and a database schema layer. Hatchify builds on top of the popular Koa web framework and can be added to any Koa app via Middleware.

Hatchify, using your database schema files, can inject REST endpoints into the Koa application providing CRUD operations, data validation, authentication tools, and more with zero configuration. Unlike many other tools all of this processing is done at runtime and does not require any code generators to function.
...
Hatchify can be integrated into any Koa Application as Middleware.
...
The only things you are required to provide are a `name` for your model and the `attributes` that will be held within it. These attributes will map into columns inside your database table. If you have written ORM models before, specifically Sequelize, this should look pretty familiar to you. Hatchify uses Sequelize, a Node.js and TypeScript compatible ORM, under the hood to talk to your database.
...
For example, if you had a new `User` model that needed special authorization rules you can quickly add this functionality yourself while still retaining all the other benefits of the Hatchify system.
...
The main Hatchify constructor can, optionally, take a database parameter that can be used to configure a large variety of relational database connections.
...
Within your custom route you can still take advantage of many Hatchify functions using the provided exports. These exported functions allow you to access:

- Hatchify Models
- Model Validation Rules
- JSON:API Serializers

Here is a more complete example using many of the Hatchify built in helpers to perform some report generation across many database tables
