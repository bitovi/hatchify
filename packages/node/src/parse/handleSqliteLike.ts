...
import { QueryStringParser } from "./builder";

export function handleSqliteLike(
    ops: QueryStringParser<FindOptions>,
    dialect: string): QueryStringParser<FindOptions> {
    ...
    const walk: <T>(
        ...
        if (Array.isArray(maybeObj)) {
            ...
            return entries.reduce((acc, [key, value]) => {
                const [newVal, newKey = key] = (fn(key, value) as unknown as [unknown, string | symbol]) ?? [];
                return {
                    ...acc,
                ...
                if (typeof key === "string" && Op.iLike in (value as any)) {
                    ...
                    if (ilClause && typeof ilClause === "object" && Op.any in ilClause) {
                        ...
                        return [
                            ...
                            (ilClause[Op.any] as string[]).map((token) => Sequelize.where(
